...
There was no justfile here at all before, and CI carried four jobs whose steps
existed nowhere else — including one that fetched actionlint through
`curl | bash` and zizmor through `pipx run`, neither pinned, to lint the very
workflows it was defined in.
...
There is no ESLint. TypeScript in strict mode, with `noUnusedLocals` and
`noUnusedParameters`, already rejects most of what a linter would catch in a
project with no runtime dependencies.
